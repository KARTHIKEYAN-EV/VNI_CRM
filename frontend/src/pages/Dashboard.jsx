...
const CARDS = {
  submitted:       { label:'Awaiting Approval',  path:'/workflow',             accent:'#818cf8', grad:'rgba(129,140,248,0.15),rgba(99,102,241,0.05)',   icon:['M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2','M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2'] },
  approved:        { label:'Ready to Dispatch',  path:'/workflow',             accent:'#34d399', grad:'rgba(52,211,153,0.12),rgba(16,185,129,0.04)',      icon:['M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8l1 12a2 2 0 002 2h8a2 2 0 002-2L19 8'] },
  dispatched:      { label:'Pending Delivery',   path:'/workflow',             accent:'#06b6d4', grad:'rgba(6,182,212,0.12),rgba(6,182,212,0.04)',        icon:'M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0' },
  delivered:       { label:'Adoption Follow-up', path:'/follow-ups',           accent:'#fbbf24', grad:'rgba(251,191,36,0.12),rgba(245,158,11,0.04)',      icon:'M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4' },
  pendingFollowUp: { label:'Overdue Follow-up',  path:'/follow-ups',           accent:'#f87171', grad:'rgba(248,113,113,0.12),rgba(239,68,68,0.04)',      icon:'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' },
  myDraft:         { label:'My Drafts',          path:'/requests?status=DRAFT', accent:'#94a3b8', grad:'rgba(148,163,184,0.1),rgba(100,116,139,0.04)',   icon:'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
}
  ...
  return (
    <button onClick={() => nav(cfg.path)}
      ...
      style={{
        ...
        padding: '20px 20px 18px', borderRadius: 18, border: '1px solid',
        borderColor: hov ? cfg.accent + '50' : 'rgba(255,255,255,0.07)',
        background: hov
    ...
    >
      ...
      <div>
        ...
        <p style={{ color: '#64748b', fontSize: 12, marginTop: 5, fontWeight: 500, letterSpacing: '0.02em' }}>{cfg.label}</p>
      </div>
    </button>
  )
}
    ...
    <Layout>
      <div style={{ padding: '28px 28px 40px', maxWidth: 900 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 11, color: '#4b5563', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Live Dashboard</span>
              </div>
              <h1 style={{
              ...
              <p style={{ color: '#334155', fontSize: 13 }}>Here's what's happening in the field today.</p>
            </div>
        ...
        {visibleCards.length > 0 && (
          <div style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Pending Actions</span>
              <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.05)' }} />
              <span style={{ fontSize: 11, color: '#1e3a5f' }}>{visibleCards.length} item{visibleCards.length > 1 ? 's' : ''}</span>
            </div>
        ...
        <div className="stagger-in d150" style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Quick Actions</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.05)' }} />
          </div>
              ...
              return (
                <button key={a.path} onClick={() => navigate(a.path)}
                  style={{
                    ...
                    background: hov ? `${a.color}12` : 'rgba(13,17,32,0.7)',
                    backdropFilter: 'blur(12px)',
                    border: `1px solid ${hov ? a.color + '40' : 'rgba(255,255,255,0.07)'}`,
                    color: hov ? a.color : '#4b5563',
                    fontSize: 13, fontWeight: 500, fontFamily: 'Inter,sans-serif',
                    transform: hov ? 'translateY(-3px)' : 'translateY(0)',
                    boxShadow: hov ? `0 10px 28px ${a.color}18` : '0 2px 10px rgba(0,0,0,0.2)',
        ...
        <div className="stagger-in d200" style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Session</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.05)' }} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px,1fr))', gap: 10 }}>
              ...
              return (
                <div key={label}
                  ...
                  style={{
                    ...
                    background: hov ? `${color}0d` : 'rgba(13,17,32,0.7)',
                    backdropFilter: 'blur(12px)',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: 10, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>{label}</span>
                  </div>
                  <p style={{
                    color: label === 'Status' ? (user?.isActive ? '#34d399' : '#f87171') : '#e2e8f0',
        ...
        <div className="stagger-in d250" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <div>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Build Progress</span>
              <p style={{ fontSize: 11, color: '#1e3a5f', marginTop: 3 }}>{doneCount} of {PHASES.length} phases complete</p>
            </div>
            <div style={{
          ...
          {/* Progress bar */}
          <div style={{ height: 6, background: 'rgba(255,255,255,0.06)', borderRadius: 99, marginBottom: 20, overflow: 'hidden', position: 'relative' }}>
            <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {PHASES.map(({ id, label, done, next, items }, i) => {
              const [hov, setHov] = useState(false)
              const dotColor = done ? '#10b981' : next ? '#fbbf24' : '#1e3a5f'
              const accentColor = done ? '#10b981' : next ? '#fbbf24' : '#1e3a5f'
              const bg = hov
